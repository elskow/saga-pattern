...
func run() (err error) {
	...
	resources, err := serverutil.BootstrapParticipant(cfg, "choreography-payment-service", "choreography-saga/payment-service/db/migrations")
	if err != nil {
		return err
	}
	defer func() {
	...
	service, err := payments.NewService(repo, messaging.NewPaymentTopicPublisher(resources.RuntimePublisher), metrics)
	if err != nil {
	...
	consumerGroup, err := commonkafka.NewSubscriberGroup(
		cfg.Runtime.KafkaBrokers, cfg.ServiceName, consumer.Topics(), resources.Logger,
