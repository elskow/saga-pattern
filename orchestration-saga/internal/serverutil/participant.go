...
func BootstrapParticipant(cfg sharedconfig.Config, migrationScope string, migrationDir string) (*ParticipantResources, error) {
...
func RunParticipantServer(cfg sharedconfig.Config, logger *slog.Logger, handler http.Handler, consumerTopics []string, replyTopic string, subscriber *commonkafka.SubscriberGroup) (err error) {
	defer func() {
	...
	logger.Info("starting orchestration participant service", "service", cfg.ServiceName, "pattern", cfg.Pattern, "addr", cfg.Address(), "consumerTopics", consumerTopics, "replyTopic", replyTopic)
