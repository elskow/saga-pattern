...
func NewMetrics(registry *prometheus.Registry) (*Metrics, error) {
	...
	stepDuration := prometheus.NewHistogramVec(
		prometheus.HistogramOpts{Name: metricStepShippingDuration, Help: "Shipping step duration in seconds.", Buckets: []float64{.005, .01, .025, .05, .1, .25, .5, 1, 2.5, 5, 10, 25, 50, 75, 100, 120}, ConstLabels: constLabels()},
		[]string{labelPattern, labelService},
	)
	compensations := prometheus.NewCounterVec(
		prometheus.CounterOpts{Name: metricCompensations, Help: "Total shipping compensations.", ConstLabels: constLabels()},
		[]string{labelPattern, labelService},
	)
